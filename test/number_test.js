(function (){
    ...
    describe('number tests', function () {
        ...
        it('number isZero 0', function () {
            expect(n0.isZero()).to.be.true;
        });
        it('number isZero 10', function () {
            expect(n1.isZero()).to.be.false;
        });
        it('number isPositive 0', function () {
            expect(n0.isPositive()).to.be.false;
        });
        it('number isPositive 10', function () {
            expect(n1.isPositive()).to.be.true;
        });
        it('number isPositive -10', function () {
            expect(n2.isPositive()).to.be.false;
        });
        it('number isNegative 0', function () {
            expect(n0.isNegative()).to.be.false;
        });
        it('number isNegative 10', function () {
            expect(n1.isNegative()).to.be.false;
        });
        it('number isNegative -10', function () {
            expect(n2.isNegative()).to.be.true;
        });
        it('number isEqual 10', function () {
            expect(n1.isEqual(10)).to.be.true;
        });
        it('number toComplex 0', function () {
            expect(n0.toComplex().isEqual(new Complex(0, 0))).to.be.true;
        });
        it('number toComplex 10', function () {
            expect(n1.toComplex().isEqual(new Complex(10, 0))).to.be.true;
        });
        it('number toComplex -10', function () {
            expect(n2.toComplex().isEqual(new Complex(-10, 0))).to.be.true;
        });
        ...
        it('complex isEqual 1+2i', function () {
            expect(z1.isEqual(new Complex(1, 2))).to.be.true;
        });
        it('complex neg 1+2i', function () {
            expect(z1.neg().isEqual(new Complex(-1, -2))).to.be.true;
        });
        it('complex conjugate 1+2i', function () {
            expect(z1.conjugate().isEqual(new Complex(1, -2))).to.be.true;
        });
        it('complex conjugate -2+i', function () {
            expect(z3.conjugate().isEqual(new Complex(-2, -1))).to.be.true;
        });
        it('complex conjugate 4-3i', function () {
            expect(z4.conjugate().isEqual(new Complex(4, 3))).to.be.true;
        });
        it('complex isReal 1+2i', function () {
            expect(z1.isReal()).to.be.false;
        });
        it('complex isReal 10', function () {
            var z = new Complex(10, 0);

            expect(z.isReal()).to.be.true;
        });
        ...
        it('complex isZero 0+0i', function () {
            expect(z0.isZero()).to.be.true;
        });
        it('complex isZero 1+2i', function () {
            expect(z1.isZero()).to.be.false;
        });
        it('complex isPositive 10+0i', function () {
            expect(new Complex(10, 0).isPositive()).to.be.true;
        });
        it('complex isPositive 0+0i', function () {
            expect(new Complex(0, 0).isPositive()).to.be.false;
        });
        it('complex isPositive -10+0i', function () {
            expect(new Complex(-10, 0).isPositive()).to.be.false;
        });
        it('complex isNegative 10+0i', function () {
            expect(new Complex(10, 0).isNegative()).to.be.false;
        });
        it('complex isNegative 0+0i', function () {
            expect(new Complex(0, 0).isNegative()).to.be.false;
        });
        it('complex isNegative -10+0i', function () {
            expect(new Complex(-10, 0).isNegative()).to.be.true;
        });
        it('complex add 1+2i 3+4i', function () {
            var z = new Complex(4, 6);
            
            expect(z1.add(z2).isEqual(z)).to.be.true;
        });
        it('complex sub 1+2i 3+4i', function () {
            var z = new Complex(-2, -2);
            
            expect(z1.sub(z2).isEqual(z)).to.be.true;
        });
        it('complex mul 1+2i 3+4i', function () {
            var z = new Complex(-5, 10);
            
            expect(z1.mul(z2).isEqual(z)).to.be.true;
        });
        it('complex div 1+2i 3+4i', function () {
            var z = new Complex(11/25, 2/25);
            
            expect(z1.div(z2).isEqual(z)).to.be.true;
        });
        it('complex lt 1 2', function () {
            var z1 = new Complex(1, 0),
                z2 = new Complex(2, 0);

            expect(z1.lt(z2)).to.be.true;
        });
        it('complex lt 1 1', function () {
            var z1 = new Complex(1, 0),
                z2 = new Complex(1, 0);

            expect(z1.lt(z2)).to.be.false;
        });
        ...
        it('complex ge 1 2', function () {
            ...
            expect(z1.ge(z2)).to.be.false;
        });
        it('complex ge 1 1', function () {
            var z1 = new Complex(1, 0),
                z2 = new Complex(1, 0);

            expect(z1.ge(z2)).to.be.true;
        });
        ...
        it('complex lt 1+0i 1', function () {
            expect(new Complex(1, 0).lt(1)).to.be.false;
        });
        it('complex lt 1+0i 0', function () {
            expect(new Complex(1, 0).lt(0)).to.be.false;
        });
        it('complex lt 1+0i -1', function () {
            expect(new Complex(1, 0).lt(-1)).to.be.false;
        });
        it('complex lt 1+0i 2', function () {
            expect(new Complex(1, 0).lt(2)).to.be.true;
        });
        it('number lt 1 1+0i', function () {
            expect((1).lt(new Complex(1, 0))).to.be.false;
        });
        it('number lt 0 1+0i', function () {
            expect((0).lt(new Complex(1, 0))).to.be.true;
        });
        it('number lt -1 1+0i', function () {
            expect((-1).lt(new Complex(1, 0))).to.be.true;
        });
        it('number lt 1 1', function () {
            expect((1).lt(1)).to.be.false;
        });
        it('number lt 0 1', function () {
            expect((0).lt(1)).to.be.true;
        });
        it('number lt -1 1', function () {
            expect((-1).lt(1)).to.be.true;
        });
        it('number add 10 -10', function () {
            expect(n1.add(n2).isEqual(0)).to.be.true;
        });
        it('number add 0 -10', function () {
            expect(n0.add(n2).isEqual(-10)).to.be.true;
        });
        it('rational toNumber 2/3', function () {
            expect(q1.toNumber().isEqual(0.6666666666666666)).to.be.true;
        });
        ...
        it('integer gcd 32 -36', function () {
            ...
            expect(z.isEqual(new Integer('4'))).to.be.true;
        });
        it('integer neg 10', function () {
            expect(i1.neg().isEqual(i2)).to.be.true;
        });
        it('integer neg -10', function () {
            expect(i2.neg().isEqual(i1)).to.be.true;
        });
        it('integer isZero 0', function () {
            expect(i0.isZero()).to.be.true;
        });
        it('integer isZero 10', function () {
            expect(i1.isZero()).to.be.false;
        });
        it('integer isZero -10', function () {
            expect(i2.isZero()).to.be.false;
        });
        it('integer isPositive 0', function () {
            expect(i0.isPositive()).to.be.false;
        });
        it('integer isPositive 10', function () {
            expect(i1.isPositive()).to.be.true;
        });
        it('integer isPositive -10', function () {
            expect(i2.isPositive()).to.be.false;
        });
        it('integer isNegative 0', function () {
            expect(i0.isNegative()).to.be.false;
        });
        it('integer isNegative 10', function () {
            expect(i1.isNegative()).to.be.false;
        });
        it('integer isNegative -10', function () {
            expect(i2.isNegative()).to.be.true;
        });
        it('integer add 0 10', function () {
            expect(i0.add(i1).isEqual(i1)).to.be.true;
        });
        it('integer add 0 -10', function () {
            expect(i0.add(i2).isEqual(i2)).to.be.true;
        });
        it('integer add 10 -10', function () {
            expect(i1.add(i2).isZero()).to.be.true;
        });
        it('integer sub 0 10', function () {
            expect(i0.sub(i1).isEqual(i2)).to.be.true;
        });
        it('integer sub 0 -10', function () {
            expect(i0.sub(i2).isEqual(i1)).to.be.true;
        });
        it('integer sub 10 -10', function () {
            expect(i1.sub(i2).isEqual(new Integer('20'))).to.be.true;
        });
        it('integer mul 0 10', function () {
            expect(i0.mul(i1).isZero()).to.be.true;
        });
        it('integer mul 0 -10', function () {
            expect(i0.mul(i2).isZero()).to.be.true;
        });
        it('integer mul 10 -10', function () {
            expect(i1.mul(i2).isEqual(new Integer('-100'))).to.be.true;
        });
        it('integer div 0 10', function () {
            expect(i0.div(i1).isZero()).to.be.true;
        });
        it('integer div 0 -10', function () {
            expect(i0.div(i2).isZero(i1)).to.be.true;
        });
        it('integer div 10 -10', function () {
            expect(i1.div(i2).isEqual(new Integer('-1'))).to.be.true;
        });
        it('integer abs 0', function () {
            expect(i0.abs().isZero()).to.be.true;
        });
        it('integer abs 10', function () {
            expect(i1.abs().isEqual(i1)).to.be.true;
        });
        it('integer abs -10', function () {
            expect(i2.abs().isEqual(i1)).to.be.true;
        });
        it('integer floorQuotient 5 2', function () {
            ...
            expect(i1.floorQuotient(i2).isEqual(i3)).to.be.true;
        });
        it('integer floorQuotient -5 2', function () {
            var i1 = new Integer('-5'),
                i2 = new Integer('2'),
        ...
        it('integer floorRemainder 5 2', function () {
            ...
            expect(i1.floorRemainder(i2).isEqual(i3)).to.be.true;
        });
        it('integer floorRemainder -5 2', function () {
            var i1 = new Integer('-5'),
                i2 = new Integer('2'),
